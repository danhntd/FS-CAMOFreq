...
from detectron2.evaluation import COCOEvaluator, inference_on_dataset, verify_results, PascalVOCDetectionEvaluator, \
    LVISEvaluator, DatasetEvaluators, CAMOEvaluator
...
class Trainer(DefaultTrainer):
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        # if evaluator_type == "coco-superclass":
        #     evaluator_list.append(
        #         SuperclassCOCOEvaluator(dataset_name, cfg, True, output_folder, classes_to_eval=classes_to_eval))
        if evaluator_type == "camo":
            evaluator_list.append(
                CAMOEvaluator(dataset_name, cfg, True, output_folder)
            )
...
def add_new_config(cfg):
    ...
    cfg.MODEL.LOSS_TYPE = 'origin'
    cfg.MODEL.TRIPLET_MARGIN = 0.5
    cfg.MODEL.TAU = 1.0
    cfg.MODEL.CAPACITY = 32
    cfg.MODEL.ALPHA_TRIPLET = 1.0
    cfg.MODEL.BETA_MEMORY = 1.0
