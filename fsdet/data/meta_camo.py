    ...
    if is_shots:
        for _, fileids_ in fileids.items():
            ...
            for i in range(len(dicts)):
                ...
                dicts[i]["file_name"] = file_name.replace(".jpg", "_v4.jpg")
                dataset_dicts.append(dicts[i])



                # dicts[i]["file_name"] = file_name.replace(".jpg", "_v1.jpg")
                # dataset_dicts.append(dicts[i])
                # # Not augmented
                # dicts[i]["aug"] = False
                # dataset_dicts.append(dicts[i])
                ...
                # dicts[i]["aug"] = True
                # dicts[i]["file_name"] = file_name.replace(".jpg", "_v1.jpg")
                # # dicts[i]["file_name_ref"] = dicts[(i + 1) % len(dicts)]["file_name"]
                # dataset_dicts.append(dicts[i])

                # # Augmented
                # dicts[i]["aug"] = True
                # dicts[i]["file_name"] = file_name.replace(".jpg", "_v2.jpg")
                # # dicts[i]["file_name_ref"] = dicts[(i + 1) % len(dicts)]["file_name"]
                # dataset_dicts.append(dicts[i])

                # # Augmented
                # dicts[i]["aug"] = True
                # dicts[i]["file_name"] = file_name.replace(".jpg", "_v3.jpg")
                # # dicts[i]["file_name_ref"] = dicts[(i + 1) % len(dicts)]["file_name"]
                # dataset_dicts.append(dicts[i])
    else:
...
def register_meta_camo(name, metadata, imgdir, annofile, sid):
